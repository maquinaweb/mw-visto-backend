...
class InspectionStepViewSet(BulkDeleteMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if hash_val and self.action in ["partial_update", "retrieve", "create", "rotate"]:
            try:
    ...
    def rotate(self, request, pk=None):
        ...
        if not step.file:
            return Response({"error": "Nenhuma imagem associada a este passo"}, status=400)
        ...
        if degrees not in [90, 180, 270]:
            return Response({"error": "Graus de rotação devem ser 90, 180 ou 270"}, status=400)
